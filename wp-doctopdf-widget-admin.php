...
if (!class_exists('WpdoctopdfwAdmin')) {
	
	class WpdoctopdfwAdmin {
		public $abs_site_root;
		public $site_http_root;
		
        function WpdoctopdfwAdmin() {
        ...
        function make_admin_page() {
			...
			if ( $_POST ) {	
				...
				foreach ((array) $fields as $key) {
					$field = trim($_POST['wp_doctopdf_widget'][$key]);
			...
			echo '<h2>'
			. __('WP WORDTOPDF Converter Widget Setup')
			. '</h2>' . "\n";
			echo '<p>You can set global options for WP WORDTOPDF Converter Widgets here. Settings can be overriden with every single converter widget instance.</p>'."\n";
			echo '<ul>
        	...
        	echo '<tr valign="top">' . "\n"
				...
				. '<select name="wp_doctopdf_widget[skin]"> ';
				
				foreach($wpdoctopdfw_object->getSkins() as $skin) {
        	...
        	echo '<tr valign="top">' . "\n"
				...
				. '</td>' . "\n"
				. '<td width="25%" style="padding:15px 0;">'
				. '<input type="text" name="wp_doctopdf_widget[max_size]" size="3" value="'.(isset($o[0]['max_size'])?$o[0]['max_size']:'').'"/> MB'
				. '</td>' . "\n"
				. '<td width="59%" rowspan="2" style="padding:15px 0;">'
				. ''
				. '</td>'
        	...
        	echo '<tr valign="top">' . "\n"
				...
				. '</td>' . "\n"
				. '<td width="25%" style="padding:15px 0;">'
				. '<input type="text" name="wp_doctopdf_widget[affid]" value="'.(isset($o[0]['affid'])?$o[0]['affid']:'').'"/> '
				. '</td>' . "\n"
				. '<td width="59%" rowspan="2" style="padding:15px 0;">'
				. '</td>'
        		. '</tr>'."\n";
        	echo '<tr valign="top">' . "\n"
				...
				. '</td>' . "\n"
				. '<td width="25%" style="padding:15px 0;">'
				. '<input type="checkbox" value="1" name="wp_doctopdf_widget[afflink]" '.(isset($o[0]['afflink'])? ($o[0]['afflink'] == 1 ? 'checked' : '') :'').'/> '
				. '</td>' . "\n"
				. '<td width="59%" rowspan="2" style="padding:15px 0;">'
				. '</td>'
        		. '</tr>'."\n";
...
if (class_exists("WpdoctopdfwAdmin")) {
	$wpdoctopdfw_admin_object = new WpdoctopdfwAdmin();
}
//Actions and Filters	
if (isset($wpdoctopdfw_admin_object)) {
